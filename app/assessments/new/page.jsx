...
export default function CreateAssessmentPage() {
	...
	async function handleSubmit(e) {
		...
		if (title.trim().length < 3) { alert('Assessment title must be at least 3 characters'); return; }
		if (!courseId) { alert('Please select a course'); return; }
		if (questions.length === 0) { alert('Please add at least one question'); return; }

		for (let i = 0; i < questions.length; i++) {
			const q = questions[i];
			if (!q.question.trim()) { alert(`Question ${i + 1}: Question text is required`); return; }
			if (q.type === 'mcq') {
				if (q.options.length < 2) { alert(`Question ${i + 1}: MCQ questions must have at least 2 options`); return; }
				if (q.options.some(opt => !opt.trim())) { alert(`Question ${i + 1}: All MCQ options must be filled`); return; }
			}
			if (q.type === 'text' && q.expectedType === 'number') {
				if (isNaN(parseFloat(q.correctAnswer))) { alert(`Question ${i + 1}: Correct answer must be a number`); return; }
			}
		}
		...
		<div className="space-y-8">
			...
			<form onSubmit={handleSubmit}>
				<div className="space-y-6">
					{/* Basic Info */}
					<Card>
						<CardHeader><CardTitle>Basic Information</CardTitle><CardDescription>Enter the assessment details</CardDescription></CardHeader>
						<CardContent className="space-y-4">
							<div>
								<label className="block text-sm font-medium mb-2">Assessment Title <span className="text-destructive">*</span></label>
								<Input value={title} onChange={(e) => setTitle(e.target.value)} placeholder="e.g., Python Fundamentals Quiz" required minLength={3} />
							</div>
							<div>
							...
							<div>
								<label className="block text-sm font-medium mb-2">Assessment Type <span className="text-destructive">*</span></label>
					...
					<Card>
						<CardHeader>
							<div className="flex items-center justify-between">
								<div><CardTitle>Questions</CardTitle><CardDescription>Add questions to your assessment</CardDescription></div>
					...
					<div className="flex gap-4 w-1/2 mx-auto justify-center">
						<Button type="submit" disabled={submitting}>
							{submitting ? <><Loader2 className="h-4 w-4 mr-2 animate-spin" /> Creating...</> : <><Save className="h-4 w-4 mr-2" /> Create Assessment</>}
						</Button>
